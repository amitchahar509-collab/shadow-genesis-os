...
- **LLM provider** — no key configured; every "intelligent" path silently falls back to rules (Anthropic adapter exists — one `ANTHROPIC_API_KEY` lifts every heuristic gate + the benchmark score).
...
- **V8 COMPLETE — 13/13 gaps closed.** Remaining work is force-multipliers/hardening, not gaps: exercise the LLM path (turns every heuristic gate into real reasoning + activates real web scanning — the single highest-value move), guard rollout to remaining write routes + usage limits, G11 plugin perf tracking, and the pre-existing standalone-build/junction env debt.
