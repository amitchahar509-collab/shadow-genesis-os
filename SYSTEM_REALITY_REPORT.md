...
- **World Scanner upgrade + Plugin marketplace** (V8 G1, G11) — not present. G1 genuinely needs a browser/search key to be non-trivial; exercising the LLM path is the single highest value-per-effort remaining task (turns every heuristic gate into real reasoning + enables real scanning). 12 of 13 V8 gaps closed.
