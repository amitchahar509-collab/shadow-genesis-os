...
- **Demand graph, marketplace/scanner/evolution engines** (V8 G1, G3, G7–G8, G11) — not present. G7 evolution completion (consumes the arena's new learning substrate) and the World Scanner upgrade (G1, needs a browser key) are the ranked-highest next steps; exercising the LLM path is the highest value-per-effort cross-cutting task.
