...
- **LLM provider** — no key configured; every LLM-gated path falls back to rules, surfaced honestly (cycle 21). As of cycle 25 there is a **multi-provider model router** (`agent-runtime/router/`): supports `ANTHROPIC_API_KEY` + `OPENROUTER_API_KEY` (Anthropic adapter unchanged), routes per agent (CEO/Board→reasoning/opus, Engineering→coding/sonnet, Research→long-context/gemini, Memory→cheap/gpt-4o-mini, else default), and falls through a chain primary→next-provider→cheap-fallback. Real token counts + ESTIMATED cost (published per-1M rates) recorded to `LlmUsage`; dashboard shows routing table + token/cost usage. Proven: real fallback walked all 3 hops across both providers (real 401s), OpenRouter adapter reaches openrouter.ai. One valid key (either provider) activates real reasoning with zero code change.
