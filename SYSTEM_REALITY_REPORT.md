...
- **Demand graph, acquisition loop, long-horizon operator, arena/feedback/marketplace/benchmark engines** (V8 G1, G3–G13) — not present. Long-Horizon Operator (G5) is the ranked-highest next step ("operate 90 days" is the last pipeline leg with no machinery), then the Acquisition loop (G4, now unblocked by the approval layer).
