...
- **Demand graph, scanner/marketplace engines** (V8 G1, G3, G8, G11) — not present. World Scanner upgrade (G1, needs a browser key) and Demand Graph (G3) are the ranked-highest next steps; exercising the LLM path is the highest value-per-effort cross-cutting task.
