...
- **Scanner/marketplace engines** (V8 G1, G8, G11) — not present. App Marketplace (G8, cheap now that Product DNA + DemandMatch exist) and the World Scanner upgrade (G1, needs a browser key) are the ranked-highest next steps; exercising the LLM path is the highest value-per-effort cross-cutting task.
