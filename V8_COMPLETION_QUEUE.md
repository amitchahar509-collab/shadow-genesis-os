...
## Re-audit after cycle 9 (next, ranked)

Safety layer now exists (G2 ✓) — external writes block until a human approves, so acquisition/growth work can land safely. Remaining ranked order: (1) **G5 Long-Horizon Operator** — "operate 90 days" is the last pipeline leg with no machinery; daily/weekly/monthly loops over active companies; (2) **G4 Acquisition loop** — now unblocked by G2 (its external actions route through the approval queue); (3) **G1 World Scanner upgrade** — discovery is the weakest gate without a browser key; (4) **G13 dashboard UI** — the intelligence + approval queue are API-only.
