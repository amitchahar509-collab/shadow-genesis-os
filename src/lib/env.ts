...
export function validateEnv(): EnvCheck[] {
  const checks: EnvCheck[] = [
    ...
    { name: "ZAI_API_KEY", required: false, set: Boolean(process.env.ZAI_API_KEY), value: process.env.ZAI_API_KEY ? "[set]" : null, message: "Optional — enables LLM-powered agents. Without it, agents use rule-based fallbacks." },
