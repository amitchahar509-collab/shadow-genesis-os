...
- **LLM provider / multi-brain router (V9)** — `OPENROUTER_API_KEY` is configured and live-proven (real completions from claude-opus-4.8, gemini-3.5-flash, gpt-4o-mini, qwen-coder, gemini-flash-lite). The V9 layer (`model-registry/` + `router/` v2 + `model-arena/`): a dynamic **ModelRegistry** (12 live OpenRouter models + 2 direct-Anthropic rows; curated tiers + MEASURED reliability/latency/duel wins; prices synced from the real catalog); registry-driven per-agent routing (CEO/Board→Opus/GPT, Engineering→best coder, WorldScanner/Research→Gemini, Memory/Customer→cheap) with importance-aware cost intelligence (LOW→cheap, CRITICAL→frontier; pre-call expected cost recorded); **Fallback 2.0** (retry-once on transient → next hop → cross-provider → emergency cheap); a **Model Arena** (same-task duels, RULE judge, ModelDuel rows) + standard models-benchmark that updates rankings; multi-model boardroom (per-seat brains via SEAT_MODELS, recorded per argument); Model Command Center dashboard panel. **Learning proven with real data**: models that failed live calls (account hit OpenRouter 402 — no credits) collapsed in measured reliability and were re-ranked out of their seats. Full 9-seat live debate awaits account credits (operational); the fallback kept every mission alive on honest heuristics meanwhile.
...
- **Plugin/Skill Marketplace** (V8 G11) — ✅ DELIVERED (cycle 22). Installable AGENT/TOOL/WORKFLOW plugins over REAL artifacts (registered agents, base tools, evolution AgentTemplates, CustomTools); performance from real AgentExecution/ToolCall usage; trust = source + performance × usage-volume confidence + benchmark; install/versioning/on-demand benchmark. Refuses to wrap non-existent modules. `plugins/`, `/api/genesis/plugins`, dashboard panel, 9 tests, e2e-proven (23 real plugins, usage-driven ranking).
- **V8 COMPLETE — 13/13 gaps + all ranked force-multipliers delivered** (LLM-path visibility cycle 21, plugin marketplace cycle 22). Remaining is operational/hardening only: set a valid `ANTHROPIC_API_KEY` (activates real reasoning + web scanning — operational, not code); guard rollout to the remaining write routes + per-org usage limits; a CI workflow + standalone-build/junction env debt. The autonomous venture network is functionally complete and honest end-to-end.
