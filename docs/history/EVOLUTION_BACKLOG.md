...
| 8 | ~~**CI pipeline**~~ — ✅ DELIVERED cycle 24. `.github/workflows/ci.yml` (Bun on Ubuntu): install → prisma generate/push (fresh SQLite) → typecheck → lint → test → build, on push + PR. Activates once a GitHub remote is added. | 5 | Easy | SRE | ~~Green workflow on push~~ ✅ (verified locally: 162/162 on a fresh DB) | Push a commit, watch workflow |
