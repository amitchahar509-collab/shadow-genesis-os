...
---
Task ID: 50
Agent: Claude Code (Fable 5) - cycle 48: V10 Module 11 - Enterprise Hardening (RBAC, isolation, backups, GDPR, compliance - all real, honest)

Task: V10 Module 11 - make Genesis enterprise-grade by EXTENDING existing systems: RBAC, workspace/tenant isolation, org policies, quotas, backups, encryption readiness, key rotation, GDPR, audit export, compliance readiness, enterprise health. Never fabricate compliance/encryption/backups/recovery/certification.

Phase 0 (reuse audit): auth layer already has guard()/Role(OWNER-ADMIN-MEMBER-VIEWER)/authenticate/createApiKey/bootstrap/audit()/checkAndRecordUsage (per-org daily quota); models AuthUser/Organization/Membership/ApiKey(sha256-hashed, revocable)/UsageCounter. Approval Engine, Security Engine, AuditLog, Company OS all present. No OrgPolicy/Backup/GDPR models. Module 11 = additive orchestration over ALL of these - zero rebuilds.

Work Log:
- Schema (+3 metadata models): OrgPolicy (quotas/retention/allowed-connectors), BackupRecord (UNCONFIGURED|COMPLETED|RESTORED + real manifest + checksum), GdprRequest (approval-gated EXPORT/DELETE).
- enterprise/rbac.ts: fine-grained Permission matrix over the existing Role (read/write/approve/execute_external/manage_members/manage_policy/manage_backups/gdpr_admin/delete_data) with min-role gates; can()/authorize()/rbacMatrix() - pure, testable.
- enterprise/index.ts: setOrgPolicy/getOrgPolicy + checkResourceQuota (reuses UsageCounter); verifyTenantIsolation (REAL scan: orphaned key/membership/counter refs, unscoped connector actions -> isolation score + violations); encryptionReadiness (VERIFIED api-keys-hashed/redaction/TLS/key-rotation, UNCONFIGURED at-rest without GENESIS_DB_ENCRYPTION_KEY); rotateApiKey (mint via createApiKey + revoke old) + secretLifecycle (key ages/staleness); createBackup (UNCONFIGURED without GENESIS_BACKUP_TARGET - never faked; real per-table row-count manifest + masked target + checksum with it) + restoreBackup + disasterRecoveryMeta; requestGdpr/decideGdpr/executeGdpr (approval-gated; EXPORT = real manifest, DELETE = real scoped deletion but PRESERVES audit trail for lawful-basis compliance); exportAuditLog; complianceReport (SOC2/ISO27001/GDPR/HIPAA - only VERIFIED controls from real state, every control cites evidence, disclaimer "NOT a certification"); enterpriseHealth (Security/Compliance/Isolation/Recovery/Reliability/Encryption scores, EACH with howComputed from real data).
- API /api/genesis/enterprise (guardWrite + RBAC authorize gates): GET health/compliance/isolation/encryption/rbac/secrets/backups/policy/audit-export; POST set-policy(OWNER)/quota-check/backup(ADMIN)/restore(ADMIN)/gdpr-request(OWNER)/gdpr-decide(ADMIN)/gdpr-execute(OWNER)/rotate-key(ADMIN). Dashboard Enterprise Control Center panel (health bars w/ how-computed tooltips, RBAC chips, secret lifecycle, backups) wired into Venture Intelligence.
- Tests (enterprise.test.ts, 14): RBAC escalation + explained authorize; isolation scores real data + flags a real violation; quota enforcement; backup UNCONFIGURED-never-faked + configured-real-manifest + restore; GDPR delete approval-gated + real deletion + rejected-never-runs + export-never-deletes; compliance verified-controls-only + no-certification + honest UNMET; encryption readiness; key rotation mints+revokes; health scores all explained; audit export. Fixed during dev: DB FKs PREVENT orphaned keys (an isolation guarantee itself) - switched the violation test to a non-FK usage-counter orphan; rotation/isolation tests create the Organization first.
